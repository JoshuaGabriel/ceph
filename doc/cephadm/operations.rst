...
You can also configure Ceph daemons to log to files instead of
...
a configuration key :confval:`mgr/cephadm/cephadm_log_destination` to the
...
destination by setting the :confval:`mgr/cephadm/cephadm_log_destination`
...
* You can manage the host(s) by running the following command:
  ...
  See :ref:`cephadm-adding-hosts` for more information.
...
* If the stray daemon(s) are running on hosts not managed by cephadm, you can
  manage the host(s) by running the following command:
...
See :ref:`cephadm-removing-hosts` for more information.
...
.. prompt:: bash #
  ...
  ceph cephadm config-check [enable|disable] <name>
...
flagged as an anomaly and a health check warning is raised.
...
health check warning is raised for any hosts that are set at a lower link speed rate.


``CEPHADM_CHECK_NETWORK_MISSING``
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

The :confval:`public_network` and :confval:`cluster_network` settings support subnet definitions
for IPv4 and IPv6. If these settings are not found on any host in the cluster,
a health check warning is raised.
...
reports any anomalies as a health check warning. *This check is bypassed if an upgrade
is in process.*
...
Management of Client Keyrings and Configs
=========================================
...
When a client keyring is placed under management, cephadm will:
  ...
  - store a copy of the ``ceph.conf`` file at ``/var/lib/ceph/<fsid>/config/ceph.conf``
    on the specified host(s)
  - store a copy of the ``ceph.client.admin.keyring`` file at ``/var/lib/ceph/<fsid>/config/ceph.client.admin.keyring``
    on the specified host(s)
...
Management of ``/etc/ceph/ceph.conf``
=====================================
...
:ref:`client_keyrings_and_configs` above).  To write config files to hosts
without client keyrings, run the following command:
...
Limiting Passwordless Sudo Access
=================================

By default, the cephadm install guide recommends enabling passwordless
...
differs one must extend the list of passwordless ``sudo`` commands prior to
upgrade.

Commands requiring passwordless sudo support:
...
.. note:: Typically cephadm will execute ``which`` to determine which ``python3``
...
.. note:: This supports only LVM-based deployed OSD(s).

.. prompt:: bash #

  ceph orch device replace <host> <device-path>

If the device being replaced is shared by multiple OSDs (e.g., DB/WAL
device shared by multiple OSDs), the orchestrator will warn you:
...
``cephadm`` will execute ``ceph-volume`` to zap and destroy all related devices
and mark the corresponding OSD as ``destroyed`` so the different OSD(s) ID(s)
will be preserved:
...
The device being replaced is finally seen as ``being replaced``
preventing ``cephadm`` from redeploying the OSDs too fast:
...
If for any reason you need to clear the 'device replace header' on a device,
run a command of the following form:

.. prompt:: bash # auto

  # ceph orch device replace <host> <device> --clear
...
Cephadm will then redeploy the OSD service spec within a few minutes
(unless the service is set to ``unmanaged``).
